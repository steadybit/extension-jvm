...
func (i *JavaProcessInspector) Inspect(p *process.Process, retries int, source string) {
	if _, err := i.scheduler.Schedule(func(ctx context.Context) {
		...
		if javaVm, err := createJvm(ctx, p, source); err == nil {
			if ctx.Err() == nil {
				i.ch <- javaVm
			}
		} else if retries > 0 {
...
func createJvm(ctx context.Context, p *process.Process, source string) (JavaVm, error) {
	containerId := container.GetContainerIdForProcess(p)
	if containerId == "" {
		return createJvmOnHost(ctx, p, source)
	}
	...
	if containerPid := container.FindMappedPidInContainer(p.Pid); containerPid > 0 {
		...
		return createJvmInContainer(ctx, p, source, containerId, containerPid)
	}

	return nil, errors.New("container pid is not available")
}

func createJvmOnHost(ctx context.Context, p *process.Process, source string) (JavaVm, error) {
	...
	if hsPerfDataPath := hsperf.FindHsperfdataFile(ctx, p); hsPerfDataPath != "" {
		...
		javaVm, err = createJvmUsingHsperfdata(ctx, p, source, hsPerfDataPath)
		if err != nil {
			return nil, err
		}
	} else {
...
func createJvmInContainer(ctx context.Context, p *process.Process, source, containerId string, pidInContainer int32) (JavaVmInContainer, error) {
	var javaVm *defaultJavaVm
	var err error
	...
	if hsperfdataFile := hsperf.FindHsperfdataFileContainer(ctx, p, pidInContainer); hsperfdataFile != "" {
		...
		javaVm, err = createJvmUsingHsperfdata(ctx, p, source, hsperfdataFile)
		if err != nil {
			return nil, err
		}
	} else {
		log.Trace().Msgf("No hsperfdata found for pid %d in container %s", pidInContainer, containerId)
	}
...
func createJvmUsingHsperfdata(ctx context.Context, p *process.Process, source, path string) (*defaultJavaVm, error) {
	...
	if err := utils.RootCommandContext(ctx, "cp", path, tempFile).Run(); err != nil {
		return nil, fmt.Errorf("error while copying perf data: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("error while reading perf data: %s", err)
	}
