...
/**
 * Spring Framework 6 variant of {@code WebClientHttpStatusAdvice}. Identical behaviour, but constructs the
 * Spring-6 {@link InjectedReactiveClientHttpResponseSpring6}. The instrumentation weaves this variant into
 * Spring 6+ targets (see SpringHttpClientStatusInstrumentation).
 */
public class WebClientHttpStatusAdviceSpring6 {
    ...
    static void exit(@Advice.Return(readOnly = false) Mono<ClientHttpResponse> response, @Advice.Enter Integer simulatedStatus) {
        if (simulatedStatus == null) {
            ...
            response = Mono.just(new InjectedReactiveClientHttpResponseSpring6(simulatedStatus));
