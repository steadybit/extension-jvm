...
public class JavaAgent {
    ...
    private static JavaAgentCommandListener cmdListener;
    ...
    public static void init(String agentArguments, Instrumentation instrumentation, ClassLoader previousAgent) throws Exception {
        if ("true".equals(System.getProperty("steadybit.javaagent.inject-bootstrap-enabled", "true"))) {
    ...
    private static void startThread(String pid, String host, String port, Instrumentation instrumentation) throws MalformedURLException {
        cmdListener = new JavaAgentCommandListener(pid, host, port, instrumentation);
        cmdListener.start();
        log.debug(String.format("JavaAgentCommandServer started for PID %s.", pid));
    }
    ...
    public static void stop() {
        if (cmdListener == null || !cmdListener.isAlive()) {
            return;
        }
        ...
        if (cmdListener.getState() == Thread.State.TIMED_WAITING) {
            cmdListener.interrupt();
        }
        try {
            cmdListener.join(5000L);
        } catch (InterruptedException ex) {
            //ignore aborted wait
            Thread.currentThread().interrupt();
        }
        if (cmdListener.isAlive()) {
    ...
    private static void injectClassesIntoBootstrapLoader(Instrumentation instrumentation) {
    ...
    private static Map<String, byte[]> locateAndFilterClasses(Instrumentation instrumentation, String... classNames) {
        ClassFileLocator classFileLocator = ClassFileLocator.ForClassLoader.of(JavaAgent.class.getClassLoader());
